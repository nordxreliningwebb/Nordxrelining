...
import KnowledgeLivePreview from '@/components/admin/KnowledgeLivePreview';
import { Save, Image as ImageIcon, Loader2 } from 'lucide-react';
import { saveKnowledgeAction } from '@/app/admin/(authenticated)/kunskapsbanken/actions';
...
export default function KnowledgeEditor({ initialData }: KnowledgeEditorProps) {
  ...
  if (initialData?.contentJson) {
    try {
      initialBlocks = typeof initialData.contentJson === 'string' ? JSON.parse(initialData.contentJson) : initialData.contentJson;
    } catch (e) {
  ...
  const uploadFile = async (file: File, path: string): Promise<string | null> => {
    try {
      ...
      const { data, error } = await supabase.storage
        .from('project-images') // Reusing the same bucket for simplicity or use 'knowledge-images' if needed
  ...
  const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const url = await uploadFile(file, 'knowledge-covers');
    if (url) {
  ...
  const handleBlockImageUpload = async (file: File): Promise<string | null> => {
    return await uploadFile(file, 'knowledge-content');
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await saveKnowledgeAction({ ...formData, contentJson });

      if (!result.success) {
        throw new Error(result.error);
      }
      setSaveStatus({ type: 'success', message: 'Inlägget har sparats framgångsrikt! Omdirigerar...' });
      
      setTimeout(() => {
        router.push('/admin/kunskapsbanken');
        router.refresh();
      }, 1500);
    } catch (err: any) {
      console.error('Save error:', err);
      setSaveStatus({ type: 'error', message: err.message || 'Ett fel uppstod när inlägget skulle sparas.' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="h-full flex flex-col font-inter">
      <div className="mb-8 flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Hantera Inlägg (Kunskapsbanken)</h1>
          <p className="mt-2 text-gray-500">Skapa och redigera inlägg med dynamiska block och Live-Preview.</p>
        </div>
        <button
          onClick={handleSave}
          disabled={isSaving}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 overflow-y-auto max-h-full scrollbar-thin scrollbar-thumb-gray-200">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Inläggets Titel</label>
                <input
                  type="text"
                  name="title"
                  ...
                  placeholder="T.ex. Vad är relining?"
                />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Ämne</label>
                <select
                  name="category"
                ...
                >
                  ...
                  <option value="Tips & Råd">Tips & Råd</option>
                  <option value="Teknik">Teknik</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Författare</label>
                <input
                  type="text"
                  name="authorName"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Utdrag / Sammanfattning</label>
              <textarea
                name="subheading"
                ...
                placeholder="En kort ingress som visas i listan..."
              />
            </div>
