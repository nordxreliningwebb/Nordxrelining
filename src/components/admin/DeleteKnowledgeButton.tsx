...
import { deleteKnowledgeAction } from '@/app/admin/(authenticated)/kunskapsbanken/actions';
import { useRouter } from 'next/navigation';

export default function DeleteKnowledgeButton({ postId }: { postId: string }) {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const res = await deleteKnowledgeAction(postId);
      if (res.success) {
        setShowModal(false);
        router.refresh();
      } else {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm transition-opacity">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all scale-100 opacity-100 flex flex-col items-center p-6 text-center">
            ...
            <p className="text-gray-600 mb-8 max-w-xs">
              Detta kommer att radera inlägget permanent från databasen. Denna åtgärd kan inte ångras.
            </p>
