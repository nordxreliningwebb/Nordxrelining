...
export async function POST(req: NextRequest) {
  try {
    ...
    const type = formData.get('type') as string || 'kontakt';
    const name = formData.get('full-name') as string || formData.get('name') as string || 'Ej angivet';
    const email = formData.get('email') as string || 'Ej angivet';
    ...
    if (type === 'offert') {
      htmlContent = `
        <h2>Ny offertförfrågan från Kalkylatorn</h2>
