...
export async function savePricePlanAction(formData: any) {
  try {
    ...
    const pricePlanData = {
      ...
      description: formData.notice_text ? `${formData.description || ""}|||NOTICE|||${formData.notice_text}` : (formData.description || ""),
...
export async function bulkSavePricePlansAction(plans: any[], category: string) {
  try {
    ...
    const plansToUpsert = plans.map(plan => ({
      ...
      name: plan.name || "",
      price: plan.price || "",
      description: plan.notice_text ? `${plan.description || ""}|||NOTICE|||${plan.notice_text}` : (plan.description || ""),
