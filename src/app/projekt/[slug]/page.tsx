...
export default async function ProjectSinglePage({ params }: ProjectPageProps) {
  ...
  const category = (project.category || 'Ej angiven');
  const catStr = category.charAt(0).toUpperCase() + category.slice(1);
  const city = project.city || 'Ej angiven';
  const dateStr = project.publish_date ? new Date(project.publish_date).toLocaleDateString('sv-SE', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Pågående';
  const bgImage = project.images?.[0] || '/construction.jpg';
  const authorAvatar = project.author_image;
  const authorName = project.author || 'Namn Saknas';
  const clientName = project.client || 'Ej angiven';
  const title = project.title || project.name || 'Utan titel';
  ...
  if (project.content) {
    try {
      blocks = typeof project.content === 'string' ? JSON.parse(project.content) : project.content;
    } catch (e) {
    ...
    <FrontendLayout>
      ...
      <main id="main-content" className="w-full" style={{ marginTop: "-85px", paddingTop: "115px" }}>
        ...
        <div className="container" style={{ maxWidth: '1400px', width: '95%', margin: '0 auto', padding: '0' }}>
            <div className="pt-10">
                <div id="dynamic-hero" className="rounded-3xl overflow-hidden relative flex flex-col justify-center items-center shadow-lg hero-anim" style={{ background: `linear-gradient(rgba(15, 23, 42, 0.6), rgba(15, 23, 42, 0.6)), url('${bgImage}')`, backgroundSize: 'cover', backgroundPosition: 'center', minHeight: '500px' }}>
        ...
        <section className="nordx-landing-cta" style={{ background: '#0284c7', color: '#ffffff', padding: '100px 20px', textAlign: 'center', position: 'relative', overflow: 'hidden', marginTop: 0, zIndex: 2, width: '100%', display: 'block' }}>
            ...
            <div className="container" style={{ maxWidth: '900px', margin: '0 auto', position: 'relative', zIndex: 2 }}>
                <h2 style={{ fontSize: 'clamp(1.5rem, 3vw, 2.5rem)', fontWeight: 800, marginBottom: '1rem', color: '#ffffff', lineHeight: 1.2 }} className="anim-mask-text">
                    <span className="anim-mask-inner">Planerar ni ett liknande projekt inom {catStr.toLowerCase()}{city !== 'Ej angiven' ? ` i ${city}` : ''}?</span>
                </h2>
                <p style={{ fontSize: '1.15rem', lineHeight: 1.6, marginBottom: '2.5rem', opacity: 0.9 }} className="anim-fade-up">
                    Vi hjälper er från idé till färdig konstruktion. Kontakta oss för en förutsättningslös diskussion.
                </p>
